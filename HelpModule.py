...
#     """
#     Compare all methods with execution timing - АДАПТИРОВАННЫЙ ВАРИАНТ
...
#     print("\n1. Neural Segmentation...")
#     neural_result_dict, neural_result, neural_mask, neural_time = apply_neural_segmentation(image_path)
...
#     # 2. Watershed (PyTorch)
#     print("\n2. Watershed (PyTorch)...")
#     watershed_result_dict, watershed_result, watershed_mask, watershed_time = apply_torch_method(
#         image_path=image_path,
#         method="watershed",
#         method_name="Watershed"
#     )
...
#     # 3. MeanShift (PyTorch)
#     print("\n3. MeanShift (PyTorch)...")
#     meanshift_result_dict, meanshift_result, meanshift_mask, meanshift_time = apply_torch_method(
#         image_path=image_path,
#         method="meanshift",
#         method_name="MeanShift",
#         bandwidth=0.5,
...
#     # 4. GrabCut (PyTorch)
#     print("\n4. GrabCut (PyTorch)...")
#     grabcut_result_dict, grabcut_result, grabcut_mask, grabcut_time = apply_torch_method(
#         image_path=image_path,
#         method="grabcut",
#         method_name="GrabCut",
#         rect=(100, 100, 200, 200),
...
#     # 5. FloodFill (PyTorch)
#     print("\n5. FloodFill (PyTorch)...")
#     floodfill_result_dict, floodfill_result, floodfill_mask, floodfill_time = apply_torch_method(
#         image_path=image_path,
#         method="floodfill",
#         method_name="FloodFill",
#         tolerance=0.15,
...
#     # 6. KMeans (PyTorch)
#     print("\n6. KMeans (PyTorch)...")
#     kmeans_result_dict, kmeans_result, kmeans_mask, kmeans_time = apply_torch_method(
#         image_path=image_path,
...
#     # 7. Sobel Edge (PyTorch)
#     print("\n7. Sobel Edge (PyTorch)...")
#     sobel_result_dict, sobel_result, sobel_mask, sobel_time = apply_torch_method(
#         image_path=image_path,
#         method="sobel_edge",
#         method_name="Sobel Edge",
#         threshold=0.1
#     )
#     methods.append("Sobel Edge")
#     results.append(sobel_result)
#     masks.append(sobel_mask)
#     times.append(sobel_time)

#     # 8. Canny Edge (PyTorch)
#     print("\n8. Canny Edge (PyTorch)...")
#     canny_result_dict, canny_result, canny_mask, canny_time = apply_torch_method(
#         image_path=image_path,
#         method="canny_edge",
...
#         high=0.3
#     )
#     methods.append("Canny Edge")
#     results.append(canny_result)
#     masks.append(canny_mask)
#     times.append(canny_time)
...
#         # Смешанный результат (50% оригинал + 50% результат)
#         try:
...
#     save_detailed_report(methods, results, masks, times, image_path)

#     return methods, results, masks, times

# def save_detailed_report(methods: List[str], results: List[Image.Image], masks: List[np.ndarray], 
#                         times: List[float], image_path: str) -> None:
#     """Сохраняет детальный отчет о сравнении"""
    
#     timestamp = datetime.datetime.now().strftime("%Y%m%d_%H%M%S")
#     report_dir = f"comparison_report_{timestamp}"
#     os.makedirs(report_dir, exist_ok=True)
    
#     print(f"\n💾 Сохранение детального отчета в {report_dir}/...")
    
#     # Сохраняем оригинальное изображение
#     original_img = Image.open(image_path).convert('RGB')
#     original_img.save(os.path.join(report_dir, "original.jpg"))
    
#     # Сохраняем результаты каждого метода
#     for i, (method, result, mask, exec_time) in enumerate(zip(methods, results, masks, times)):
...
#         # Создаем индивидуальный график для метода
#         fig, axes = plt.subplots(1, 3, figsize=(15, 5))
...
#         axes[1].set_title(f'{method} Result\n({exec_time:.3f}s)')
#         axes[1].axis('off')
...
#         axes[2].set_title(f'{method} Mask')
#         axes[2].axis('off')
        
#         plt.suptitle(f'{method} Segmentation Analysis', fontsize=14)
#         plt.tight_layout(rect=[0, 0.03, 1, 0.95])
        
#         method_plot_path = os.path.join(method_dir, f"{method.replace(' ', '_').lower()}_analysis.jpg")
#         plt.savefig(method_plot_path, dpi=150, bbox_inches='tight')
#         plt.close()
...
#                 elif mask.ndim == 3:
#                     # Для цветных масок считаем ненулевые пиксели в любом канале
#                     mask_area = np.sum(np.any(mask > 0, axis=2))
            
#             report_data.append({
#                 'Method': method,
#                 'Time (s)': exec_time,
#                 'Mask Area': mask_area,
#                 'Mask Shape': str(mask.shape) if mask is not None else 'N/A',
#                 'Success': mask is not None and mask.size > 0
#             })
        
#         df = pd.DataFrame(report_data)
#         csv_path = os.path.join(report_dir, "comparison_report.csv")
#         df.to_csv(csv_path, index=False)
#         print(f"📄 CSV отчет сохранен: {csv_path}")
        
#         # Создаем текстовый отчет
#         txt_path = os.path.join(report_dir, "comparison_report.txt")
#         with open(txt_path, 'w', encoding='utf-8') as f:
#             f.write("="*70 + "\n")
#             f.write("ОТЧЕТ О СРАВНЕНИИ МЕТОДОВ СЕГМЕНТАЦИИ\n")
#             f.write("="*70 + "\n\n")
...
#             f.write("-"*70 + "\n")
#             f.write(f"{'Метод':<20} {'Время (с)':<12} {'Площадь маски':<15} {'Статус':<10}\n")
#             f.write("-"*70 + "\n")
...
#                 exec_time = times[idx]
#                 mask_area = 0
#                 if masks[idx] is not None and masks[idx].size > 0:
#                     if masks[idx].ndim == 2:
#                         mask_area = np.sum(masks[idx] > 0)
#                     elif masks[idx].ndim == 3:
#                         mask_area = np.sum(np.any(masks[idx] > 0, axis=2))
                
#                 status = "Успешно" if masks[idx] is not None and masks[idx].size > 0 else "Ошибка"
#                 f.write(f"{method:<20} {exec_time:<12.3f} {mask_area:<15,} {status:<10}\n")
            
#             f.write("\n" + "="*70 + "\n")
#             f.write("СТАТИСТИКА:\n")
#             f.write("-"*70 + "\n")
#             f.write(f"Самый быстрый: {methods[sorted_indices[0]]} ({times[sorted_indices[0]]:.3f} с)\n")
#             f.write(f"Самый медленный: {methods[sorted_indices[-1]]} ({times[sorted_indices[-1]]:.3f} с)\n")
#             f.write(f"Среднее время: {np.mean(times):.3f} с\n")
#             f.write(f"Стандартное отклонение: {np.std(times):.3f} с\n")
        
#         print(f"📝 Текстовый отчет сохранен: {txt_path}")
        
#     except ImportError:
#         print("⚠️ Pandas не установлен. Пропускаем создание CSV отчета.")
#         print("Установите: pip install pandas")
    
#     print(f"✅ Детальный отчет сохранен в папке: {report_dir}")
...
def legacy_compare_segmentation_methods(image_path: str) -> Dict[str, Any]:
    """
    Аналог оригинальной функции compare_segmentation_methods
    но использующий TorchSegmenter
    """
    return original_compare_segmentation_methods(image_path)


def interactive_floodfill_demo(image_path):
    """
    Интерактивная демонстрация FloodFill с разными параметрами
    """
    ...
    # FloodFill с разными параметрами
    for i, tolerance in enumerate(tolerances):
        row = (i + 1) // 3
        col = (i + 1) % 3

        center_point = (w // 2, h // 2)
        result, mask = flood_fill_torch(image_tensor, center_point, tolerance)
        result_pil = transforms.ToPILImage()(result.cpu())

        axes[row, col].imshow(result_pil)
        axes[row, col].set_title(f'FloodFill (tolerance={tolerance})')
        axes[row, col].axis('off')

        # Отмечаем точку заливки
        axes[row, col].plot(center_point[0], center_point[1], 'ro',
