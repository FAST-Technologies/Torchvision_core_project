...
from sklearn.metrics import (
    ...
    jaccard_score, confusion_matrix, silhouette_score, calinski_harabasz_score, davies_bouldin_score, mean_absolute_error
)
...
class SegmentationMetrics:
    ...
    def calculate_jaccard_sklearn(
        ...
        try:
            score = jaccard_score(gt_binary.ravel(), pred_binary.ravel(), zero_division=0.0)
            return float(score)
        except ValueError as e:
    ...
    def calculate_precision_recall(
        ...
        # Sklearn (Эталон)
        p_sklearn = precision_score(gt_binary.ravel(), pred_binary.ravel(), zero_division=0)
        r_sklearn = recall_score(gt_binary.ravel(), pred_binary.ravel(), zero_division=0)
    ...
    def calculate_all_metrics(
        ...
        iou_sklearn = SegmentationMetrics.calculate_jaccard_sklearn(pred_mask, gt_mask, threshold)
        metrics['iou'] = iou_custom
        metrics['jaccard_score'] = iou_sklearn
        
        if verbose_comparison:
            print(f"IoU Check: Custom={iou_custom:.6f} | Sklearn={iou_sklearn:.6f} | Diff={abs(iou_custom-iou_sklearn):.2e}")
        ...
        if include_hausdorff:
            metrics['hausdorff_distance'] = SegmentationMetrics.calculate_hausdorff_distance(pred_mask, gt_mask, threshold)
