...
"""Модуль классических методов сегментации изображений на базе OpenCV.

Предоставляет класс `OpenCVSegmenter`, реализующий 50+ алгоритмов сегментации,
сгруппированных по 8 категориям:

┌─────────────────────────────────────────────────────────┐
│ Категория              │ Методов │ Примеры               │
├─────────────────────────────────────────────────────────┤
│ Пороговые (14)         │ 14      │ Otsu, Adaptive, Niblack, Sauvola, Phansalkar │
│ Детекторы границ (10)  │ 10      │ Canny, Sobel, Scharr, LoG, DoG, Phase Congruency │
│ Региональные (3)       │ 3       │ Region Growing, Split-and-Merge, FloodFill │
│ Кластеризация (3)      │ 3       │ K-Means, DBSCAN, MeanShift │
│ Активные контуры (4)   │ 4       │ Snakes, GVF, Morphological Snakes, Chan-Vese │
│ Watershed/графовые (2) │ 2       │ Watershed, Random Walker │
│ Суперпиксели (3)       │ 3       │ SLIC, Felzenszwalb, QuickShift │
│ Интерактивные (1)      │ 1       │ GrabCut │
└─────────────────────────────────────────────────────────┘
...
- ⚡ Оптимизации: векторизованные операции, `cv2.boxFilter` для локальных статистик,
...
Workflow:
1. Создать экземпляр: `OpenCVSegmenter(method="otsu_thresholding", **params)`
2. Выполнить сегментацию: `mask = segmenter.segment(image)`
3. (Опционально) Визуализировать: `overlay, mask = segmenter.segment_with_mask(image, alpha=0.7)`
4. (Опционально) Оценить качество: `metrics = segmenter.evaluate_metrics(pred_mask, gt_mask)`
...
class OpenCVSegmenter(BaseSegmenter):
    ...
    def _opencv_threshold_niblack(self, img: ImageArray, **kwargs: Any) -> MaskArray:
        ...
        mean: FloatArray = cv2.boxFilter(gray.astype(np.float32), cv2.CV_32F, (window_size, window_size))
        # E[X²] — среднее квадратов
        mean_sq: FloatArray = cv2.boxFilter((gray.astype(np.float32) ** 2), cv2.CV_32F, (window_size, window_size))
    ...
    def _opencv_threshold_bernsen(self, img: ImageArray, **kwargs: Any) -> MaskArray:
        ...
        window_size: int = int(self.params.get("window_size", 15))
        contrast_threshold_raw: float = float(self.params.get("contrast_threshold", 0.15))
        ...
        img_range: float = float(gray.max() - gray.min())
        is_normalized: bool = img_range <= 1.0
        if not is_normalized and contrast_threshold_raw <= 1.0:
            contrast_threshold: float = contrast_threshold_raw * 255.0
        else:
            contrast_threshold = contrast_threshold_raw
    ...
    def _opencv_threshold_phansalkar(self, img: ImageArray, **kwargs: Any) -> MaskArray:
        ...
        # Вычисление локальных статистик
        mean: FloatArray = cv2.boxFilter(gray.astype(np.float32), cv2.CV_32F, (window_size, window_size)).astype(
            np.float32
        )
        mean_sq: FloatArray = cv2.boxFilter((gray.astype(np.float32) ** 2), cv2.CV_32F, (window_size, window_size)).astype(
            np.float32
        )
        variance: FloatArray = np.maximum(mean_sq - mean**2, 0)
        std: FloatArray = np.sqrt(variance)
