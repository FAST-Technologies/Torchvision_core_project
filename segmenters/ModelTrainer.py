...
def train_deeplab_ade20k(
    ...
    for epoch in range(epochs):
        ...
        if epoch == 5:
            ...
            trainer.scheduler = torch.optim.lr_scheduler.CosineAnnealingLR(trainer.optimizer, T_max=len(train_loader) * (epochs - 5))
...
def train_fcn_resnet50_ade20k(
    ...
    for epoch in range(epochs):
        ...
        if val_miou > trainer.best_miou:
            trainer.best_miou = val_miou
            torch.save(model.state_dict(), f"./../models/fcn_{variant.replace('fcn_', '')}_ade20k_best.pth")
            print(f"   💾 Saved best model (mIoU: {val_miou:.4f})")
