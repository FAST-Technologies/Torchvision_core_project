...
class TrainingConfig:
    ...
    def __repr__(self):
        return (f"TrainingConfig({self.experiment_name}, "
...
class ModelTrainer:
    ...
    def create_model(
        ...
        # ========== TORCHVISION МОДЕЛИ ==========
            ...
            raise ValueError(f"Unknown model_type: {model_type}. Available: unet_smp, fpn_smp, psp_smp, deeplab_tv, fcn_tv, segnet")
    ...
    def train_experiment(
        ...
        for epoch in range(config.epochs):
            ...
            if is_torchvision_model and epoch == 5:
                ...
                trainer.scheduler = torch.optim.lr_scheduler.CosineAnnealingLR(
                    trainer.optimizer, 
                    T_max=len(train_loader) * (config.epochs - 5)
                )
    ...
    def compare_trained_models(
        ...
        if model_types is None:
            model_types = ["unet_smp", "deeplab_tv", "fpn_smp", "psp_smp", "fcn_tv", "segnet"]
        ...
        if checkpoint_paths is None:
            ...
            for model_type in model_types:
                ...
                pattern = os.path.join(self.checkpoint_dir, f"{model_type}_{augmentation_level}_*.pth")
    ...
    def compare_augmentations(
        ...
        comparison_path = os.path.join(
            self.checkpoint_dir,
            f"augmentation_comparison_{model_type}_{timestamp}.csv"
        )
    ...
    def plot_experiment_comparison(self, output_path: str = None):
        ...
        for result in self.experiment_results[:2]:  # Показываем первые 2
            ...
            ax3.plot(epochs, result['history']['train_loss'], 
                    label=f"{result['augmentation_level']} (Train)", 
                    linestyle='-', linewidth=2)
            ax3.plot(epochs, result['history']['val_loss'], 
        ...
        if output_path is None:
            ...
            output_path = os.path.join(
                self.checkpoint_dir,
                f"experiment_comparison_{timestamp}.png"
            )
    ...
    def train_all_models(
        ...
        model_configs = [
            ...
            {'model_type': 'fcn_tv', 'encoder_name': 'resnet34', 'variant': 'fcn_resnet50'},
        ...
        summary_path = os.path.join(
            self.checkpoint_dir,
            f"all_models_summary_{timestamp}.csv"
        )
    ...
    def evaluate_checkpoints(
        ...
        eval_path = os.path.join(
            self.checkpoint_dir,
            f"checkpoint_evaluation_{timestamp}.csv"
        )
