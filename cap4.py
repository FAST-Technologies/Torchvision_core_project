...
onnx_seg = ONNXSegmenter("otsu_thresholding", "./data/backend_comparison/otsu_thresholding.onnx", device="cuda")
...
print(f"✅ ONNX segment_with_mask: result shape={result.shape}, mask shape={mask.shape if mask is not None else None}")
...
print(f"✅ TRT segment_with_mask: result shape={result.shape}, mask shape={mask.shape if mask is not None else None}")
...
df = tester.benchmark_methods(image=img, n_runs=1, test_name="quick_test", save_benchmark=False)
