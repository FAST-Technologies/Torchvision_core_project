...
def run_performance_benchmark(
    ...
    for img_name, (img_path, img_pil, gt_mask) in test_images.items():
        ...
        # Загрузка cold результатов для сравнения
        # cold_pattern: str = f"./data/segmentation_tester_results/benchmark_{img_name}_cold_*/statistics/benchmark_results.csv"
        # cold_files: List[str] = glob.glob(cold_pattern)
        # if not cold_files:
        #     print(f"   ⚠️  Не найден cold-файл для {img_name}")
        #     continue

        # df_cold_loaded: pd.DataFrame = pd.read_csv(cold_files[0])
