...
def _log_inference_details_standalone(
    ...
    print(f"   Predicted classes ({len(unique_classes)}): {unique_classes[:20]}{'...' if len(unique_classes) > 20 else ''}")
    ...
    try:
        ...
        export_class_report(report, f"./data/ade20k_test_trained/{model_type}_prediction_report.md", format="markdown")
...
INFERENCE_STRATEGIES = {
    ...
    "fpn_se_resnext": lambda model, processor, image, device: infer_smp_model_fixed(model, image, device, output_stride=32),
    ...
    "deeplab_smp": lambda model, processor, image, device: infer_smp_model_fixed(model, image, device, output_stride=16),
    "segnet": lambda model, processor, image, device: infer_unet_smp(model, processor, image, device=device, output_stride=1),
    "segnet_custom": lambda model, processor, image, device: infer_unet_smp(model, processor, image, device=device, output_stride=1),
