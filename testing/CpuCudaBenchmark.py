...
class CpuCudaBenchmark:
    ...
    def benchmark_all_methods(
        ...
        for method_name, segmenter in methods_dict.items():
            ...
            print(f"      CPU: {cpu_result['mean_time']*1000:.2f}ms ± {cpu_result['std_time']*1000:.2f}ms")
            ...
            if torch.cuda.is_available():
                ...
                print(f"      CUDA: {cuda_result['mean_time']*1000:.2f}ms ± {cuda_result['std_time']*1000:.2f}ms")
        ...
        with open(report_path, 'w', encoding='utf-8') as f:
            ...
            for method in df['method'].unique():
                ...
                if not cpu_data.empty:
                    f.write(f"  CPU:  {cpu_data['mean_time'].values[0]*1000:.2f}ms ± {cpu_data['std_time'].values[0]*1000:.2f}ms\n")
                
                cuda_data = method_data[method_data['device'] == 'cuda']
                if not cuda_data.empty:
                    f.write(f"  CUDA: {cuda_data['mean_time'].values[0]*1000:.2f}ms ± {cuda_data['std_time'].values[0]*1000:.2f}ms\n")
        ...
        if torch.cuda.is_available():
            ...
            if cpu_vals:
                ...
                for i, method in enumerate(method_labels):
                    plt.annotate(method, (cpu_vals[i], cuda_vals[i]), 
