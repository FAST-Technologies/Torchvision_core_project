...
def _create_segmenter(
    ...
    if library == "opencv":
        ...
        if any(k in params for k in ["precision", "use_compile", "compile_mode"]):
            return TorchSegmenter2(method, **params)
        else:
            # Fallback на старый TorchSegmenter для обратной совместимости
